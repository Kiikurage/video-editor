...
function applyProps(base: PIXI.Container, props: PixiProps) {
    const { x, y, width, height, canvasScale, canvasLeft, canvasTop, selected, locked } = props;

    base.x = (0 - canvasLeft) * canvasScale;
    base.y = (0 - canvasTop) * canvasScale;
    ...
    frame.clear();
    frame.lineStyle(1, 0x4d90fe, selected ? 1 : 0);
    frame.drawRect(x * canvasScale, y * canvasScale, width * canvasScale, height * canvasScale);
    frame.hitArea = new PIXI.Rectangle(x * canvasScale, y * canvasScale, width * canvasScale, height * canvasScale);

    for (const { name, cursor, dirX, dirY } of DIR) {
        const resizer = base.getChildByName(name) as PIXI.Graphics;
        resizer.x = x * canvasScale - RESIZER_SIZE / 2;
        resizer.y = y * canvasScale - RESIZER_SIZE / 2;
        ...
        resizer.drawRect(
            (dirX === -1 ? 0 : dirX === 1 ? width : width / 2) * canvasScale,
            (dirY === -1 ? 0 : dirY === 1 ? height : height / 2) * canvasScale,
            RESIZER_SIZE,
            RESIZER_SIZE
        );
        resizer.endFill();
...
const ResizePixiView = CustomPIXIComponent(
    {
...
export function ResizeView(props: Props): React.ReactElement {
    const {
        x: originalX,
        y: originalY,
        width: originalWidth,
    ...
    const { scale, left, top } = usePreviewCanvasViewportInfo();
